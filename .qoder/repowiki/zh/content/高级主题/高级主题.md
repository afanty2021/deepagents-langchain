...
### 组件F：文件系统后端的安全路径解析与搜索优化（FilesystemBackend）
- 安全路径解析：
  - 当 `virtual_mode=True` 时，路径被视为虚拟绝对路径，禁止路径遍历（`..` 或 `~`），并确保解析路径在根目录内。
  - 使用 `O_NOFOLLOW` 标志防止符号链接遍历，增强文件读写安全性。
  - 拒绝 Windows 绝对路径（如 `C:/...`），确保路径格式一致性。
- 搜索优化：
  - 优先使用 ripgrep 进行高效搜索，失败时回退到 Python 原生实现。
  - 支持 glob 模式过滤文件，结合 `wcmatch.glob` 实现复杂匹配。
  - 限制单个文件大小（默认 10MB），避免内存溢出。
- 关键路径参考
  - [FilesystemBackend._resolve_path](file://libs/deepagents/deepagents/backends/filesystem.py#L60-L88)
  - [FilesystemBackend.grep_raw/_ripgrep_search/_python_search](file://libs/deepagents/deepagents/backends/filesystem.py#L308-L419)
  - [FilesystemBackend.glob_info](file://libs/deepagents/deepagents/backends/filesystem.py#L421-L480)

```mermaid
flowchart TD
Start["开始路径解析"] --> IsVirtual{"virtual_mode=True?"}
IsVirtual --> |是| CheckTraversal{"包含 .. 或 ~?"}
CheckTraversal --> |是| Reject["拒绝路径"]
CheckTraversal --> |否| Resolve["解析为绝对路径"]
Resolve --> CheckRoot{"在根目录内?"}
CheckRoot --> |否| Reject
CheckRoot --> |是| ReturnPath["返回安全路径"]
IsVirtual --> |否| IsAbsolute{"绝对路径?"}
IsAbsolute --> |是| ReturnPath
IsAbsolute --> |否| ResolveRelative["相对路径解析"]
ResolveRelative --> ReturnPath
```

图表来源
- [libs/deepagents/deepagents/backends/filesystem.py](file://libs/deepagents/deepagents/backends/filesystem.py#L60-L88)
- [libs/deepagents/deepagents/backends/filesystem.py](file://libs/deepagents/deepagents/backends/filesystem.py#L308-L419)
