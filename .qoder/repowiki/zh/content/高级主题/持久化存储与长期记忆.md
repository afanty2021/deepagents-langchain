...
## 更新摘要
**变更内容**
- 新增对 `AgentMemoryMiddleware` 的详细说明，包括用户级和项目级记忆的加载流程、路径配置和更新建议
- 扩展了 `agent_memory.py` 中关于长期记忆机制的文档描述
- 更新了记忆加载与保存流程的配置示例
...
### AgentMemoryMiddleware（长期记忆中间件）
**Section sources**
- [agent_memory.py](file://libs/deepagents-cli/deepagents_cli/agent_memory.py#L170-L329)
- [config.py](file://libs/deepagents-cli/deepagents_cli/config.py#L116-L329)

`AgentMemoryMiddleware` 是 DeepAgents 实现跨会话长期记忆的核心组件，通过在会话开始时加载用户级和项目级记忆文件，并将其注入系统提示，实现记忆的持久化与上下文继承。

#### 记忆加载流程
- **初始化**：在 `__init__` 方法中，通过 `Settings` 实例获取用户和项目路径配置
- **用户记忆路径**：`~/.deepagents/{assistant_id}/agent.md`
- **项目记忆路径**：`{project_root}/.deepagents/agent.md`（优先）或 `{project_root}/agent.md`
- **加载时机**：`before_agent` 钩子在每次会话开始时触发，动态检查文件是否存在以捕获用户更新
- **状态注入**：将读取的记忆内容注入 `AgentMemoryState`，供后续构建系统提示使用

#### 路径配置与管理
- **用户级路径**：由 `Settings.get_user_agent_md_path()` 生成，基于 `~/.deepagents/{agent_name}/agent.md`
- **项目级路径**：由 `Settings.get_project_agent_md_path()` 生成，基于当前 Git 项目根目录下的 `.deepagents/agent.md`
- **路径验证**：`Settings._is_valid_agent_name()` 确保 agent 名称仅包含字母、数字、连字符、下划线和空格，防止路径注入
...
- **立即更新场景**：
  ...
  - 出现编码风格、约定或工作流模式时
- **存储决策**：
  - **用户级记忆**：存储通用行为、编码偏好、通信风格等跨项目适用的信息
  - **项目级记忆**：存储项目特定的架构、约定、测试策略、部署流程等
- **操作命令**：
  ```bash
  # 用户记忆操作
  ls ~/.deepagents/{agent}/
  read_file '~/.deepagents/{agent}/agent.md'
  edit_file '~/.deepagents/{agent}/agent.md' ...
  
  # 项目记忆操作（推荐）
  ls {project_root}/.deepagents/
  read_file '{project_root}/.deepagents/agent.md'
  edit_file '{project_root}/.deepagents/agent.md' ...
  ```

#### 系统提示构建
- `_build_system_prompt` 方法将用户记忆和项目记忆整合到系统提示中
- 注入 `LONGTERM_MEMORY_SYSTEM_PROMPT` 提供记忆使用指南
- 包含详细的文件操作说明和优先级建议
...
AMW->>FS : 读取用户记忆 ~/.deepagents/{agent}/agent.md
FS-->>AMW : 用户记忆内容
AMW->>FS : 读取项目记忆 {project}/.deepagents/agent.md
FS-->>AMW : 项目记忆内容
AMW->>RT : 返回 state 更新 user_memory/project_memory
Agent->>AMW : wrap_model_call 构建系统提示
AMW-->>Agent : 返回增强后的系统提示
```

图表来源
- [agent_memory.py](file://libs/deepagents-cli/deepagents_cli/agent_memory.py#L209-L244)
- [agent_memory.py](file://libs/deepagents-cli/deepagents_cli/agent_memory.py#L246-L293)
- [config.py](file://libs/deepagents-cli/deepagents_cli/config.py#L203-L226)
