...
- 安全与特性
  - **虚拟模式**：通过 `root_dir` 和 `cwd` 限定根目录，启用 `virtual_mode` 时禁止路径穿越（如 `..` 或 `~`），确保解析路径始终在根目录内。所有路径操作均以虚拟路径（以 `/` 开头）进行，增强沙箱安全性。
  - **防符号链接**：在文件读写操作中使用 `O_NOFOLLOW` 标志（若操作系统支持），防止符号链接遍历攻击。该机制应用于 `read`、`write`、`edit`、`upload_files` 和 `download_files` 方法，确保文件访问不被符号链接重定向。
  - **搜索策略**：优先使用 `ripgrep`（`rg`）进行搜索，利用其高性能和 JSON 输出格式。若 `ripgrep` 不可用或超时，则回退到 Python 正则表达式遍历，同时受 `max_file_size_bytes` 限制，避免处理过大文件。搜索支持 `glob` 过滤，且在虚拟模式下保持路径一致性。
  - **列表与路径处理**：支持虚拟路径与真实路径两种模式，`ls_info` 和 `glob_info` 在虚拟模式下返回以 `/` 开头的虚拟路径，保持稳定排序。
- 关键方法
